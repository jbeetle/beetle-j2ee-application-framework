...
public class GenImp2 {
	...
	public void genAttributes() { //
		...
		sb.append("\"]");
		sb.append("\n");
		sb.append("    private TableOperator<"
				+ Common.genTableClassName(className) + "> operator;");
		sb.append("\n");
	}
	...
	public void genConstructor() {
		sb.append("\n");
		sb.append("    public Ps").append(Common.fisrtCharToUpCase(className))
				.append("(){");
		sb.append("\n");
		sb.append("        operator = new TableOperator<"
				+ Common.genTableClassName(className)
				+ ">(\"SYSDATASOURCE_DEFAULT\", \"" + className + "\", "
				+ Common.genTableClassName(className) + ".class);");
		sb.append("\n");
		//
		sb.append("     ////针对包含自增字段的表，使用下面的构造函数\n");
		sb.append("        //operator = new TableOperator(\"SYSDATASOURCE_DEFAULT\", \""
				+ className
				+ "\", "
				+ Common.fisrtCharToUpCase(className)
				+ ".class,\"IDENTITY_FIELD_NAME\");");
		sb.append("\n");
	...
	public void genQueryMethod_demo() {
		...
		sb.append("\n");
		sb.append("        ")
				.append("query.setSql(\"select \"+operator.generateFieldsString()+\" from \"+operator.getTableName());");
		sb.append("\n");
		sb.append("        ")
				.append("query.addParameter(new SqlParameter(SqlType.")
				.append(type.getSqlType(attributes.get(
						attributes.get("primaryKey")).toString()))
				.append(", id));");
		sb.append("\n");
		...
		sb.append("                ").append(Common.delAcross(className))
				.append(" = new ").append(Common.genTableClassName(className))
				.append("();");
		sb.append("\n");
		for (int i = 0; i < fields.size(); i++) {
			...
			sb.append("                ")
					...
					.append(".set")
					.append(Common.fisrtCharToUpCase3(fields.get(i).toString()))
					.append("(rs.getFieldValueAs")
					.append(fillerType(fieldTypes.get(i))).append("(\"")
