...
public class DB2SeqGenerator implements ISequence {
	private static DB2SeqGenerator generator = new DB2SeqGenerator();

	private DB2SeqGenerator() {
	...
	public long nextSequenceNum(SeqType seqtype) {
		...
		qo.setSql("NEXT VALUE FOR " + seqtype.getSequenceName());
		try {
			qo.access();
			...
			e.printStackTrace();
			throw new java.lang.RuntimeException("get db2 sequence err", e);
		}
		return r;
	}

	public SeqImpType getImpType() {
		return SeqType.SeqImpType.DB2;
	}
