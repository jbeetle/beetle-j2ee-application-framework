...
public class PostgreSqlSeqGenerator implements ISequence {
	...
	public long nextSequenceNum(SeqType seqtype) {
		...
		qo.setSql("select nextval('" + seqtype.getSequenceName() + "')");
		try {
			qo.access();
			...
			e.printStackTrace();
			throw new java.lang.RuntimeException("get PostgreSql sequence err",
					e);
		}
		return r;
	}
